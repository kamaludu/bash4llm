 ...
 * **Estensioni Deterministiche**: Validazione sintattica della risposta (`--validate-sml` per SML v2.0 e `--validate-regex <expr>`), sanitizzazione ANSI zero-eval (`--sanitize`), diagnostica JSON strutturata (`--json-diagnostics`) e nuova costante d'errore canonica `13` (`BASH4LLM_ERR_PARSE`).
