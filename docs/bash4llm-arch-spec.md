...
Prima di consentire qualunque elaborazione, lo script core verifica la presenza nel `PATH` dei seguenti **23 binari ed utility essenziali**. L'assenza di almeno uno di essi causa l'arresto immediato dello script con codice di stato `15`:
...
Il runtime di Bash4LLM⁺ impone regole di isolamento e protezione dei dati persistenti e temporanei per prevenire attacchi di tipo privilege escalation, race condition, hijacking dell'ambiente o directory traversal:
...
Questa macro-sezione gestisce l'inizializzazione primaria della shell, la bonifica dell'ambiente, l'analisi preventiva degli argomenti CLI, la convalida dell'ambiente operativo, la sicurezza dei percorsi e l'esposizione delle funzioni fondamentali di logging, codifica e I/O. È suddivisa in **7 Sezioni Piatte**.
...
Questa macro-sezione gestisce la persistenza a lungo termine, la rotazione dello storico, i manifesti per allegati multimodali, l'engine centrale di rete `_exec_curl_secure` e il motore unificato di gestione dei thread NDJSON. È suddivisa in **5 Sezioni Piatte**.
...
* **Funzioni**: `_normalize_bool_env`, `_exec_curl_secure`.
* **Attività**: Inizializzazione delle variabili di stato globale e definizione della **funzione centrale di mediazione di rete `_exec_curl_secure()`**, che garantisce l'isolamento dei token di autenticazione in file di header temporanei `0600` e il reindirizzamento via File Descriptor (`/dev/fd/3`) per eliminare secret leak in `argv`.
...
Questa macro-sezione racchiude l'implementazione del provider Groq integrato nel Core.
...
* **`call_api_groq`**: Chiamata HTTP sincrona non-streaming inoltrata tramite `_exec_curl_secure()`: isola la chiave API in un file di intestazione temporaneo sicuro con reindirizzamento di File Descriptor e salva la risposta in `$RESP`.
* **`call_api_streaming_groq`**: Connessione streaming SSE via `_exec_curl_secure()`: elabora il flusso in tempo reale con `tee` e `jq --unbuffered`, invia i token a `stdout`, compila il JSON sintetico finale in `$RESP` e aggiorna `last_api.json`.
* **`validate_key_groq`**: Test diagnostico rapido (timeout 10s) sull'endpoint `/models` tramite `_exec_curl_secure()` per convalidare la chiave API.
* **`auto_select_model_groq` / `validate_model_groq` / `refresh_models_groq`**: Gestiscono l'auto-selezione della prima scelta valida, la verifica e la sincronizzazione locale del catalogo modelli in `$MODELS_FILE` tramite `_exec_curl_secure()`.

---

## SEZIONE 7: CORE_SETUP

Gestisce il parsing dei parametri da riga di comando (CLI), l'interfaccia di dispatching, il whitelisting, la registrazione delle guardie `readonly -f` e la risoluzione preventiva delle azioni. È suddivisa in **7 Sezioni Piatte**.

### 7.1 Mappatura Funzionale per Sezione in CORE_SETUP

#### Sezione 1: `CORE_SETUP_DISPATCH_HELPERS`
* **Funzioni**: `validate_provider_interface`, `call_provider`, `validate_provider_key_dispatch`, `refresh_models_dispatch`, `validate_model_dispatch`, `auto_select_model_dispatch`, `_lock_security_guards`.
